...
class ChatClient {
  ...
  public async sendTyping(conversationId: string, isTyping = true): Promise<boolean> {
    // Try WebSocket first
    if (this.socket && this.connected) {
    ...
    try {
      ...
      const response = await fetch(
        `${CHAT_BASE_URL}/api/chats/${conversationId}/typing`,
        {
          method: 'POST',
          headers,
...
async function handleSendTyping(args: unknown): Promise<string> {
