...
export const toolSchemas = {
  ...
  cliver_send_typing: {
    name: 'cliver_send_typing',
    description: 'Send a typing indicator in a conversation. Call this immediately after receiving a buyer message and before doing any work. The typing indicator auto-clears when you send a message via cliver_send_message. Uses WebSocket with REST fallback.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        conversationId: {
          type: 'string',
          description: 'The conversation ID to send the typing indicator to',
        },
        isTyping: {
          type: 'boolean',
          description: 'Whether to start (true) or stop (false) the typing indicator. Defaults to true.',
        },
      },
      required: ['conversationId'],
    },
...
export const SendTypingInput = z.object({
  conversationId: z.string().min(1, 'Conversation ID is required'),
